...
describe('RegistrySingleton', () => {
	...
	it('should set tracker for all events in registry', () => {
		...
		RegistrySingleton.registerEvent(new InputEvent());
		RegistrySingleton.registerEvent(new ClickEvent());
		RegistrySingleton.setTrackerFuncForAllEventsInRegistry(tracker);
		RegistrySingleton.getEvent('input').trackElementOnTrail('some trail');
		expect(tracker).toHaveBeenCalledTimes(1);
		expect(tracker).toHaveBeenLastCalledWith('some trail');
		RegistrySingleton.getEvent('click').trackElementOnTrail('some other trail');
	...
	it('should set trail for all events in registry', () => {
		...
		RegistrySingleton.registerEvent(new ClickEvent());
		RegistrySingleton.registerEvent(new InputEvent());
		RegistrySingleton.setTrailFuncForAllEventsInRegistry(trail);
		RegistrySingleton.getEvent('click').makeTrailForElement(firstElement);
		expect(trail).toHaveBeenCalledTimes(1);
		expect(trail).toHaveBeenLastCalledWith(firstElement);
		RegistrySingleton.getEvent('input').makeTrailForElement(secondElement);
